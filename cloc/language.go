...
func (ls Languages) Less(i, j int) bool {
	if ls[i].Data.Code == ls[j].Data.Code {
		return ls[i].Data.Name < ls[j].Data.Name
	}
	return ls[i].Data.Code > ls[j].Data.Code
}
...
func (langs *DefinedLanguages) GetFormattedString() string {
	...
	for _, lang := range langs.Langs {
		printLangs = append(printLangs, lang.Data.Name)
	}
